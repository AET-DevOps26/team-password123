...
class AnalyticsServiceTest {
  ...
  void emptyMealsProduceZeroTotals() {
    LocalDate date = LocalDate.of(2026, 5, 29);
    when(mealsClient.listForUser(eq(TOKEN), eq(date), eq(date))).thenReturn(List.of());
    when(goals.findByUserId(USER_ID)).thenReturn(Optional.empty());
  ...
  void weeklyQueriesSevenDayRangeAndSumsMeals() {
    ...
    when(mealsClient.listForUser(eq(TOKEN), eq(weekStart), eq(weekEnd)))
        .thenReturn(
            List.of(
                meal(700, 50, 80, 25, 6), meal(300, 20, 30, 10, 4), meal(1000, 60, 120, 35, 10)));
    when(goals.findByUserId(USER_ID)).thenReturn(Optional.empty());

    AnalyticsResponse response = service.weekly(USER_ID, TOKEN, weekStart);
  ...
  void weeklyDoesNotThrowSinceEndIsAlwaysAfterStart() {
    LocalDate weekStart = LocalDate.of(2026, 5, 25);
    when(mealsClient.listForUser(any(), any(), any())).thenReturn(List.of());
    when(goals.findByUserId(USER_ID)).thenReturn(Optional.empty());
  ...
  void streakCountsConsecutiveUtcDaysEndingToday() {
    ...
    when(mealsClient.loggedDates(eq(TOKEN), eq(today.minusDays(1830)), eq(today)))
        .thenReturn(List.of(today, today.minusDays(1), today.minusDays(2)));

    assertThat(service.computeStreak(USER_ID, TOKEN).streak()).isEqualTo(3);
  }

  @Test
  void streakGraceRuleKeepsStreakWhenTodayNotLoggedYet() {
    LocalDate today = LocalDate.now(ZoneOffset.UTC);
    when(mealsClient.loggedDates(any(), any(), any()))
        .thenReturn(List.of(today.minusDays(1), today.minusDays(2)));

    assertThat(service.computeStreak(USER_ID, TOKEN).streak()).isEqualTo(2);
  }
  ...
  void streakIsZeroWhenLastLoggedDayIsBeforeYesterday() {
    ...
    assertThat(service.computeStreak(USER_ID, TOKEN).streak()).isZero();
  }

  @Test
  void streakIsZeroWhenNothingLogged() {
    when(mealsClient.loggedDates(any(), any(), any())).thenReturn(List.of());
  ...
  void dailyConsultsBothMealsClientAndGoalsAndNeverSaves() {
    ...
    service.daily(USER_ID, TOKEN, date);

    verify(mealsClient).listForUser(eq(TOKEN), eq(date), eq(date));
