...
class GoalServiceTest {
    ...
    private static GoalRequest request(double cal, double protein, double carbs, double fat, double fiber) {
    ...
    void upsertInsertsNewGoalWhenNoneExists() {
        when(goals.findByUserId(USER_ID)).thenReturn(Optional.empty());
        when(goals.save(any(NutritionGoal.class))).thenAnswer(invocation -> invocation.getArgument(0));
