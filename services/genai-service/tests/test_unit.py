"""Unit tests for nutrition_analyzer core logic.

These tests do NOT require a running service, Ollama, or any external API.
They test the parsing, calculation, normalization, and caching logic in isolation
using lightweight fakes and monkeypatching.
...
def _make_analyzer() -> NutritionAnalyzer:
    """Build a NutritionAnalyzer without touching Ollama or any external service."""
    a = NutritionAnalyzer.__new__(NutritionAnalyzer)
    a.provider = "ollama"
    a.nutrition_db = NutritionAnalyzer._load_nutrition_db()
    a.llm = None
    a.fallback_llm = None
    a.nutrition_client = NutritionDataClient(a.nutrition_db, provider="local")
    return a


def _tiny_image_b64() -> str:
    """Return a 1×1 white PNG as base64 — just enough for the analyzer pipeline."""
...
def _fake_response(content: str):
    """Create a fake LLM response object with a .content attribute."""
    return type("FakeLLMResponse", (), {"content": content})()
...
class TestNormalizeGrams:
    ...
    def test_boundary_exactly_at_10_not_snapped(self):
        result = self.a._normalize_grams("chicken", 10.0)
        assert result == 10.0
...
class TestParseResponse:
    ...
    def test_zero_grams_item_filtered_out(self):
...
# ---------------------------------------------------------------------------
# analyze() — full pipeline with mocked LLM and nutrition lookup
# ---------------------------------------------------------------------------
...
class TestAnalyzePipeline:
    ...
    def _run(self, llm_json: str) -> NutritionResponse:
        self.a._call_llm = lambda *args, **kwargs: _fake_response(llm_json)
    ...
    def test_two_foods_names_in_result(self):
        result = self._run(
            '{"foods": [{"food": "chicken", "grams": 100}, {"food": "rice", "grams": 100}], "confidence": 0.85}'
        )
        assert "chicken" in result.foods
        assert "rice" in result.foods
    ...
    def test_llm_none_raises_exception(self):
        self.a._call_llm = lambda *args, **kwargs: None
        with pytest.raises(Exception):
            self.a.analyze(_tiny_image_b64())

    def test_mixed_known_unknown_foods(self):
        # known: rice 100g (130 kcal) + unknown: xyz 100g (0 kcal)
        result = self._run(
...
class TestNutritionDataClient:
    ...
    def test_partial_match(self):
        result = self._client().lookup("green apple")
        assert result is not None
