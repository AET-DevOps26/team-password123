...
/**
 * PLACEHOLDER stand-in for the real GenAI meal recognition service (tracked on a
 * separate branch). This performs NO image recognition: it deterministically maps
 * the image bytes to a fixed catalog entry via a stable hash, so the same image
 * always yields the same dish, macros, and confidence. Swap this @Component for a
 * real MealAnalyzer implementation when the GenAI service lands.
 */
@Component
public class HeuristicMealAnalyzer implements MealAnalyzer {

    private static final List<Dish> CATALOG = List.of(
    ...
    private record Dish(String name, BigDecimal calories, BigDecimal protein, BigDecimal carbs, BigDecimal fat) {
        Dish(String name, int calories, int protein, int carbs, int fat) {
            this(name, BigDecimal.valueOf(calories), BigDecimal.valueOf(protein),
